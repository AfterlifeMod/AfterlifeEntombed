...
public class DivineBoonHandler {
    ...
    private static final int BLESSING_CHANCE = 140000;

    // Chance per tick for a curse to occur (1 in 72000 = ~1 per hour of playtime per player)
    private static final int CURSE_CHANCE = 720000;
    ...
    private static void triggerBlessing(ServerPlayer player) {
        ...
        // Send packet to open selection GUI on client ONLY if gamerule is on
        if (player.level().getGameRules().getBoolean(ModGameRules.RULE_ENABLE_BOONS_AND_CURSES)) {
    ...
    private static String getBlessingParticle(GodType god) {
        return switch (god) {
            ...
            case GEB ->
                "earth_aura";
            case THOTH ->
                "spiral_up"; // Knowledge/wisdom flash
            case ANUBIS ->
                "anubis_nuke"; // Death energy
            default ->
                null;
        };
    }
    ...
    private static String getCurseParticle(GodType god) {
        ...
        return switch (god) {
            case SETH ->
                "seth_fog";
            case RA ->
                "ra_column"; // Dark haze instead of bright halo
            case HORUS ->
                "shield_wall"; // Oppressive wall instead of protective shield
            case SHU ->
                "shu_jump2"; // Heavy air instead of light wind
            case ISIS ->
                "healing_burst"; // Corrupted magic
            case GEB ->
                "earth_aura"; // Crushing earth
            case THOTH ->
                "spiral_up"; // Confusion/forgetfulness
            case ANUBIS ->
                "blood_lance"; // Death curse
            default ->
                null;
        };
    }
